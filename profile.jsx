...
const Profile = (props) => {
	...
	useEffect(() => {
		axios.get(global.config.apiurl + `uploadimg/countimgs`, { withCredentials: true })
			.then((res) => {
				let alldata = res.data;
				// alert(alldata[0].totalimgs);
				setTotalimgs(alldata[0].totalimgs);
			});
		axios.get(global.config.apiurl + `uploadimg/countcontests`, { withCredentials: true })
			.then((res) => {
				let alldata = res.data;
				// console.log(alldata);
		...
		const imgsdata = [];
		axios.get(global.config.apiurl + `uploadimg/byUser`)
			.then((res) => {
				let alldata = res.data;
				alldata.map(function (val, i, arr) {
					imgsdata.push({
						src: val.url, thumbnail: val.url, thumbnailWidth: 640,
						thumbnailHeight: 320,
					})
				});
